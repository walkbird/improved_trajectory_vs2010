			...
			for (list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				if(iTrack->index >= trackInfo.length) {
					...
					if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {
						...
						printf("%f\t", min<float>(max<float>(mean_x/float(seqInfo.width), 0), 0.999));
						printf("%f\t", min<float>(max<float>(mean_y/float(seqInfo.height), 0), 0.999));
						printf("%f\t", min<float>(max<float>((frame_num - trackInfo.length/2.0 - start_frame)/float(seqInfo.length), 0), 0.999));

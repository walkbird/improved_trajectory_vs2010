...
void DenseSample(const Mat& grey, vector<Point2f>& points, const double quality, const int min_distance)
{
...
void InitPry(const Mat& frame, vector<float>& scales, vector<Size>& sizes)
{
...
void DrawTrack(const vector<Point2f>& point, const int index, const float scale, Mat& image)
{
...
void PrintDesc(vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo)
{
...
void LoadBoundBox(char* file, vector<Frame>& bb_list)
{
	// load the bouding box file
    ifstream bbFile(file);
    string line;
...
void InitMaskWithBox(Mat& mask, vector<BoundBox>& bbs)
{
	int width = mask.cols;
...
void ComputeMatch(const vector<KeyPoint>& prev_kpts, const vector<KeyPoint>& kpts,
...
void MergeMatch(const vector<Point2f>& prev_pts1, const vector<Point2f>& pts1,
				const vector<Point2f>& prev_pts2, const vector<Point2f>& pts2,
				vector<Point2f>& prev_pts_all, vector<Point2f>& pts_all)
{
	prev_pts_all.clear();
...
void MatchFromFlow(const Mat& prev_grey, const Mat& flow, vector<Point2f>& prev_pts, vector<Point2f>& pts, const Mat& mask)
{
	...
	for(int i = 0; i < prev_pts.size(); i++) {
		int x = min<int>(max<int>(cvRound(prev_pts[i].x), 0), width-1);
		int y = min<int>(max<int>(cvRound(prev_pts[i].y), 0), height-1);
